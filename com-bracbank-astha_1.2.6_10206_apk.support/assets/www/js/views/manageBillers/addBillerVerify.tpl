<%
	...
	var inputLabelArr = els.get("inputLabelArr");
	inputValueArr.reverse();
	inputLabelArr.reverse();
%>
<div class="pagewrapper" id="appwrapper" >
	...
	<header class="appheader">
		<div class="headerLeft">
			...
			<a href="#/addbiller" class="bacArr">
      			<i class="fa fa-angle-left" aria-hidden="true"></i>
    			</a>
	      </div>
	      <div class="headerMiddle">
  			<h4  class="p10">Add Biller Review</h4>	
		</div>
			...
			<section class="content">    
        		<form method="post" action="#">	
            		...
            		<ul class="list-unstyled list-nobrd">
						<% if(els.get("selectedBillerTypeName") != "Electricity" ||  els.get("selectedBillerTypeName") != "Water") {%>
						...
						<% if(els.get("selectedBillerListName") == "DPDC" || els.get("selectedBillerListName") == "DESCO" || els.get("selectedBillerListName") == "NESCO") { %>
							<li class="list-group-item">
	                    		...
	                    		<p><%- els.get("billerCustomerName")%></p>
                    		</li>
						<% } %>
                		<!--<li class="list-group-item">
                    		<label>Biller Service</label>
                    		<p><%- els.get("selectedBillerServiceType")%></p>
                		</li>-->
            			<% _.each(inputLabelArr, function(value,index) { %>
            				<li class="list-group-item">
	                    		<label><%- value %></label>
	                    		<p><%- inputValueArr[index]%></p>
	                		</li>
            			<% }); %>
        			...
        			<div class="row">
                		<div class="col-xs-6 pr5">
                    		<button type="button" class="btn btn-default btn-block" id="addBillerVerifyCancelBtn">Cancel</button>
                		</div>
                		<div class="col-xs-6 pl5">
                    		<button type="button" class="btn btn-primary btn-block" id="addBillerVerifyConfirmBtn">Confirm</button>
                		</div>
            		</div> 
            		<br>   
        		</form>
			        ...
			        <form method="post" action="#">
			            <div class="modal-content">
		                	<div class="modal-header">
		                  		<button type="button" class="close" id="addBillerOTPCloseBtn" data-dismiss="modal" aria-label="Close">
			                ...
			                <div class="modal-body">
			                    ...
			                    <div class="form-group">
									...
									<% if(els.get("alertType") == "SMS" || els.get("alertType") == "EMAIL") { %>
										...
										<p class="text-center small">
											<a href="javascript:void(0);" class="resend_button" id="resend_btn12"><%-$.i18n.t('app.transfer.managebeneficiary.resendotp')%></a>
										</p>
									<% } %>
			                    </div>
			                    <div class="modal-footer">
			                        <div class="row">
										<div class="col-xs-6 pr5">
											<button type="button" id="addBillerOTPCancel" class="btn btn-primary cancel-btn btn-block" data-dismiss="modal">Cancel</button>
										</div>
										<div class="col-xs-6 pr5">
											<button type="button" class="btn btn-default btn-block otp_done" id="addBillerOTPVerify"><%-$.i18n.t('app.transfer.managebeneficiary.verify')%></button>
										</div>
									</div>
			                    </div>
