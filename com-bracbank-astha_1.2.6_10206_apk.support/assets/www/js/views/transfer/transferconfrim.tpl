<%
	...
	console.log("selectedAccountDTO",selectedAccountDTO);
	var paymentType = els.get("paymentType");
	var paymentAmount = els.get("paymentAmount");
	var purposetransfer = els.get("purposetransfer");
	...
	console.log("fromAccName",els.get("fromAccName"));
...
%>
	...
	<header class="appheader">
		<div class="headerLeft">
			...
			<a href="javascript:void(0);" id="OwnCard_Verify_BackBtn" class="bacArr">
				<i class="fa fa-angle-left" aria-hidden="true"></i>
			</a>
	      </div>
	      <div class="headerMiddle">
  			<h4 class="p10">
				<%-$.i18n.t('app.transferss.review')%>
    ...
    <form method="post" action="#">	
        ...
        <div class="veriWrap">
            <div class="media">
                ...
                <% if(els.get("ccPayFromAcc") == "bKash") { %>
            		<div class="media-body media-middle">
	                    <%- els.get("fromAccNumber")  %>
	                </div>
                <% } else { %>
                	 <div class="media-body media-middle">
	                    <%- els.get("fromAccName")  %>
	                    <p class="small text-muted"><%- els.get("fromAccNumber")%></p>
	                </div>
                <% } %>
            ...
            <div class="media m0">
                ...
                <div class="media-body media-middle">
                    <span class="amt pos">
	                    <%if(paymentType == "otheramt") {%>
	                   		<%- checkAmount($.formatNumber(els.get("transferAmount"), {format:"#,###.00", locale:"us"}))%>
	             		<%} else{%>
	                 		<%- paymentAmount %>
	                  	<% } %>
            ...
            <div class="media m0">
                ...
                <div class="media-body media-middle">
                    ...
                    <%- selectedAccountDTO.productName%></small>
                    <p class="small text-muted"><%- selectedAccountDTO.accountNumber%></p>
                    
                </div>
                <div class="media-right text-right media-middle">
                    <span class="cur small text-muted"><%- els.get("toCardCurr") %></span>
                </div>
            </div>
        </div>
        <ul class="list-unstyled list-nobrd">
        	<% if(els.get("ccPayFromAcc") == "bKash") { %>
        		<li class="list-group-item">
					...
					<p>
						<%- checkAmount($.formatNumber(els.get("bkashAddMoneyTotFee"), {format:"#,###.00", locale:"us"}))%> BDT
					</p>
				</li>
				<li class="list-group-item">
					<label><%-$.i18n.t('app.transferss.totaldebitamount')%></label>
					<p>
						<%- checkAmount($.formatNumber(els.get("bkashAddMoneyTotAmt"), {format:"#,###.00", locale:"us"}))%> BDT
					</p>
				</li>
				<%if(els.get("fromAccCurr") !== els.get("toCardCurr")) {%>
					<li class="list-group-item">
						<label><%-$.i18n.t('app.transferss.exchangeratelabel')%></label>
						<p>
							1 <%- els.get("toCardCurr")%> = <%- checkAmount($.formatNumber(els.get("bkashAddMoneytoExchangeRate"), {format:"#,###.00", locale:"us"}))%> <%- els.get("fromAccCurr")%>
						</p>
					</li>
					<%if(els.get("bKashAddMoneydebitAmount") != "" && els.get("bKashAddMoneydebitAmount") != null) {%>
						<li class="list-group-item">
							<label><%-$.i18n.t('app.transferss.debitamount')%></label>
							<p>
								<%- checkAmount($.formatNumber(els.get("bKashAddMoneydebitAmount"), {format:"#,###.00", locale:"us"}))%> <%- els.get("fromAccCurr")%>
							</p>
						</li>
					<% } %>
					<% if(els.get("bKashAddMoneycreditAmount") != "" && els.get("bKashAddMoneycreditAmount") != null) {%>
						<li class="list-group-item">
							<label><%-$.i18n.t('app.transferss.creditamount')%></label>
							<p>
								<%- checkAmount($.formatNumber(els.get("bKashAddMoneycreditAmount"), {format:"#,###.00", locale:"us"}))%> <%- els.get("toCardCurr")%>
							</p>
						</li>
					<% } %>
				<% } %>
        	<% } else { %>
				...
				<%if(els.get("fromAccCurr") !== els.get("toCardCurr")) {%>
					<li class="list-group-item">
						<label><%-$.i18n.t('app.transferss.exchangeratelabel')%></label>
						<p>
							1 <%- els.get("toCardCurr")%> = <%- checkAmount($.formatNumber(els.get("toExchangeRate"), {format:"#,###.00", locale:"us"}))%> <%- els.get("fromAccCurr")%>
						</p>
					</li>
					...
					<% if(els.get("creditAmount") != "" && els.get("creditAmount") != null) {%>
						<li class="list-group-item">
							<label><%-$.i18n.t('app.transferss.creditamount')%></label>
							<p>
								<%- checkAmount($.formatNumber(els.get("creditAmount"), {format:"#,###.00", locale:"us"}))%> <%- els.get("toCardCurr")%>
							</p>
						</li>
					<% } %>
				<% } %>
        ...
        <div class="row">
            ...
            <button type="button" id="OwnCard_Verify_CancelBtn" class="btn btn-default btn-block"><%-$.i18n.t('app.transferss.cancel')%></button>
            </div>
            <div class="col-xs-6 pl5">
           <button type="button" class="btn btn-primary btn-block" id="OwnCard_Verify_TransferBtn"><%-$.i18n.t('app.transferss.transfer')%></button>
            </div>
        </div>
    </form>
</section>
		            ...
		            <form method="post" action="#">
		                <div class="modal-content">
		                    <div class="modal-header">
		                        <button type="button" class="close" id="ownCard_OTPCloseBtn" data-dismiss="modal" aria-label="Close">
		                    ...
		                    <div class="modal-body">
								...
								<% if(els.get("alertType") == "SMS" || els.get("alertType") == "EMAIL") { %>
									...
									<p class="text-center small">
										<a href="javascript:void(0);" class="resend_button" id="resend_btn21"><%-$.i18n.t('app.transfer.managebeneficiary.resendotp')%></a>
									</p>
								<% } %>
		                    </div>
		                    <div class="modal-footer">
		                        <div class="row">
									<div class="col-xs-6 pr5">
										<button type="button" id="ownCardOTPCancel" class="btn btn-primary cancel-btn btn-block" data-dismiss="modal">Cancel</button>
									</div>
									<div class="col-xs-6 pr5">
										<button type="button" class="btn btn-default btn-block otp_done" id="ownCardOTPVerify"><%-$.i18n.t('app.transfer.managebeneficiary.verify')%></button>
									</div>
								</div>
		                    </div>
		                </div>
