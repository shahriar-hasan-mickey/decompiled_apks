...
<div class="pagewrapper" id="appwrapper" >
	...
	<header class="appheader">
		<div class="headerLeft">
			...
			<a href="#/wallettobeneinit" class="bacArr">
  				<i class="fa fa-angle-left" aria-hidden="true"></i>
    		</a>
	      </div>
	      <div class="headerMiddle">			
			<h4 class="p4"><%- els.get("walletType")%> Transfer Review<p class="small">(FundTransfer)</p></h4>
		</div>
			...
			<section class="content">
    			<form>	
        			...
        			<div class="veriWrap">
            			<div class="media">
                			...
                			<div class="media-right text-right media-middle">
                    			<span class="cur small text-muted"><%- els.get("walletFromAccCur")%></span>
                			</div>
            			</div>
			            <div class="media">
                			<div class="media-left media-middle">
            			...
            			<div class="media">
                			...
                			<div class="media-body media-middle">
                    			<%- selectedWalletBeneInfo.shortName%>
                    			<p class="small text-muted"><%- selectedWalletBeneInfo.accountNumber%></p>
                			</div>
            			</div>
        			</div>
        			<ul class="list-unstyled list-nobrd">
						...
						<%if(els.get("walletFromAccCur") !== els.get("walletToAccCur")) { %>
							<li class="list-group-item">
								<label><%-$.i18n.t('app.transferss.exchangeratelabel')%></label>
								<p>
									1 <%- els.get("walletToAccCur")%> = <%- checkAmount($.formatNumber(els.get("toExchangeRate"), {format:"#,###.00", locale:"us"}))%> <%- els.get("walletFromAccCur")%>
								</p>
							</li>
							<%if(els.get("debitAmount") != "" && els.get("debitAmount") != null) {%>
								<li class="list-group-item">
									...
									<p>
										<%- checkAmount($.formatNumber(els.get("debitAmount"), {format:"#,###.00", locale:"us"}))%> <%- els.get("walletFromAccCur")%>
									</p>
								</li>
							<% } %>
							<% if(els.get("creditAmount") != "" && els.get("creditAmount") != null) {%>
								<li class="list-group-item">
									...
									<p>
										<%- checkAmount($.formatNumber(els.get("creditAmount"), {format:"#,###.00", locale:"us"}))%> <%- els.get("walletToAccCur")%>
									</p>
								</li>
							<% } %>
						<% } %>
    				...
    				<div class="row">
            			<div class="col-xs-6 pr5">
                			<button type="button" class="btn btn-default btn-block" id="walletToBeneVerifyCancel"><%-$.i18n.t('button.login.cancel')%></button>
            			</div>
            			<div class="col-xs-6 pl5">
                			<button type="button" class="btn btn-primary btn-block" id="walletToBeneConfirmBtn"><%-$.i18n.t('button.transfer.sendmoney')%></button>
            			</div>
        			</div>
    			</form>
			</section>
			        ...
			        <form method="post" action="#">
			            <div class="modal-content">
			                <div class="modal-header">
			                  <button type="button" class="close" id="walletToBeneOTPCloseBtn" data-dismiss="modal" aria-label="Close">
			                ...
			                <div class="modal-body">
			                    ...
			                    <div class="modal-footer">
			                        <div class="row">
										<div class="col-xs-6 pr5">
											<button type="button" id="walletToBeneOTPCancel" class="btn btn-primary cancel-btn btn-block" data-dismiss="modal">Cancel</button>
										</div>
										<div class="col-xs-6 pr5">
											<button type="button" class="btn btn-default btn-block otp_done" id="walletToBeneOTPVerify"><%-$.i18n.t('app.transfer.managebeneficiary.verify')%></button>
										</div>
									</div>
			                    </div>
