<%
	...
	if(fromAccType == "CASA") {
		selectedFromAccInfo = casaListArray[selectedInd];
	} else if(fromAccType == "CC") {
		selectedFromAccInfo = ccListArray[selectedInd];
	} else {
		selectedFromAccInfo = pcListArray[selectedInd];
	}
		...
		<div class="headerLeft">
			<button type="button" id="mnutoggle" class="btn btn-default mnutoggle">
				...
				<span class="icon-bar"></span>
                  ...
                  <a href="javascript:void(0);" id="walletOneOffVerifyBackBtn" class="bacArr">            
           			<i class="fa fa-angle-left" aria-hidden="true"></i>
            	</a>
	      </div>
	      <div class="headerMiddle">
  			<h4  class="p4"><%- els.get("walletType")%> Transfer Review<p class="small">(Fund Transfer)</p></h4>	
		</div>
			...
			<section class="content">
    			<form>	
        			...
        			<div class="veriWrap">
            			<div class="media">
                			...
                			<div class="media-body media-middle">
                    			<%- formAccName%>
                    			<p class="small text-muted"><%- selectedFromAccInfo.accountNumber%></p>
                			</div>
                			<div class="media-right text-right media-middle">
                    			<span class="cur small text-muted"><%- els.get("walletFromAccCurr")%></span>
                			</div>
            			</div>
            			<div class="media">
                			<div class="media-left media-middle">
            			...
            			<div class="media">
                			...
                			<div class="media-body media-middle">
                    			<%- els.get("walletToAccName")%>
                    			<p class="small text-muted"><%- els.get("walletToAccNum")%></p>
                			</div>
        				</div>
        			</div>
        			<ul class="list-unstyled list-nobrd">
						...
						<% if(!isNull(els.get("bankFee"))) { %>
							<li class="list-group-item">
								<label>Bank Fees</label>
								<p><%- els.get("bankFee")%></p>
								<% if(!isNull(els.get("bankDescription"))) { %>
									<p class="small"><%- els.get("bankDescription")%></p>
								<% } %>
							</li> 
						<% } %>
						<% if(els.get("walletType") == "Rocket") { %>
							<% if(!isNull(els.get("rocketFee"))) { %>
								<li class="list-group-item">
									<label>Rocket Fees</label>
									<p><%- els.get("rocketFee")%></p>
								</li> 
							<% } %>
						<% } %>
							<li class="list-group-item">
							...
							<%if(els.get("walletFromAccCurr") !== els.get("walletToAccCurr")) { %>
								<li class="list-group-item">
									<label><%-$.i18n.t('app.transferss.exchangeratelabel')%></label>
									<p>
										1 <%- els.get("walletToAccCurr")%> = <%- checkAmount($.formatNumber(els.get("toExchangeRate"), {format:"#,###.00", locale:"us"}))%> <%- els.get("walletFromAccCurr")%>
									</p>
								</li>
								<%if(els.get("debitAmount") != "" && els.get("debitAmount") != null) {%>
									<li class="list-group-item">
										...
										<p>
											<%- checkAmount($.formatNumber(els.get("debitAmount"), {format:"#,###.00", locale:"us"}))%> <%- els.get("walletFromAccCurr")%>
										</p>
									</li>
								<% } %>
								<% if(els.get("creditAmount") != "" && els.get("creditAmount") != null) {%>
									<li class="list-group-item">
										...
										<p>
											<%- checkAmount($.formatNumber(els.get("creditAmount"), {format:"#,###.00", locale:"us"}))%> <%- els.get("walletToAccCurr")%>
										</p>
									</li>
								<% } %>
							<% } %>
							<% if(!isNull(els.get("walletRemarks"))) { %>
								<li class="list-group-item">
									<label><%-$.i18n.t('app.transferss.transreason')%></label>
									<p><%- els.get("walletRemarks")%></p>
								</li>
							<% } %>
        			</ul>
        			...
        			<div class="row">
            			<div class="col-xs-6 pr5">
                			<button type="button" class="btn btn-default btn-block" id="walletOneTimeVerifyCancel"><%-$.i18n.t('button.login.cancel')%></button>
            			</div>
            			<div class="col-xs-6 pl5">
                			<button type="button" class="btn btn-primary btn-block" id="walletOneTeVerifySendBtn"><%-$.i18n.t('button.transfer.sendmoney')%></button>
            			</div>
        			</div>
    			</form>
			</section>
			        ...
			        <form method="post">
			            <div class="modal-content">
			                <div class="modal-header">
			                  <button type="button" class="close" id="walletOneTimeOTPCloseBtn" data-dismiss="modal" aria-label="Close">
			                ...
			                <div class="modal-body">
			                    ...
			                    <div class="form-group">
									...
									<% if(els.get("alertType") == "SMS" || els.get("alertType") == "EMAIL") { %>
										...
										<p class="text-center small">
											<a href="javascript:void(0);" class="resend_button" id="resend_btn11"><%-$.i18n.t('app.transfer.managebeneficiary.resendotp')%></a>
										</p>
									<% } %>
			                    </div>
			                    <div class="modal-footer">
			                        <div class="row">
										<div class="col-xs-6 pr5">
											<button type="button" id="walletOneTimeOTPCancel" class="btn btn-primary cancel-btn btn-block" data-dismiss="modal">Cancel</button>
										</div>
										<div class="col-xs-6 pr5">
											<button type="button" class="btn btn-default btn-block otp_done" id="walletOneTimeOTPVerify"><%-$.i18n.t('app.transfer.managebeneficiary.verify')%></button>
										</div>
									</div>
			                    </div>
