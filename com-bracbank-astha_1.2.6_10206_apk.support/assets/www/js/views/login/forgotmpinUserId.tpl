			                     ...
			                     <!-- fieldset Starts here-->
			                        <fieldset>
					                           ...
					                           <% if ( devicePlatform == "Android" ) { %>
					                                <input type="text" autocomplete='off' autocorrect="off" maxlength="30" class="form-control restrictCpyPst" id="username" name="username" placeholder="<%-$.i18n.t('app.mpin.enteruserid')%>" >
					                           <%}else{%>
					                           	<input type="text" autocomplete='off' autocorrect="off" onkeydown="call2(event)" maxlength="30" class="form-control restrictCpyPst" id="username" name="username" placeholder="<%-$.i18n.t('app.mpin.enteruserid')%>" >
					                           <%}%>
					                             </div>
					                             <div class="form-group required">
					                             <% if ( devicePlatform == "Android" ) { %>
			                             ...
			                             <input type="button" class="btn btn-success btn-block" id="validateuserid"  value="<%-$.i18n.t('app.mpin.submit')%>" />
...
function call2(e) {
        ...
        var userName = $('#userid').val();
        	for (var i=0;i< userName.length;i++){
  	          		var atchar = userName[i];
  	          		var atcharnxt = userName[i+1];
  						if(atchar === atcharnxt){
  							if(atchar === userName[i+2]){
		            		 if (e.keyCode == 32) {
					                e.preventDefault();
					                return false;
					            }
