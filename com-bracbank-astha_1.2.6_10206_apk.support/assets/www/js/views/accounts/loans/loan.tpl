<%
	...
	var loanAccountListDTO = els.get("loanAccountList");
	var loanListArray = "";
	if(!isNull(loanAccountListDTO)) {
		loanListArray = loanAccountListDTO.filter(function(array) {
	...
	<aside class="mainnav">
  		<div class="text-center userInfo">
    		...
    		<div>
      			...
      			<p class="small info"><%-$.i18n.t('app.accounts.lastlogin1')%> <%-els.get("lastSuccessfulLogin")%></p>
            	...
            	<div class="userInfo">
          		...
          		<div class="dashTab">
					<ul class="nav nav-tabs" role="tablist">
						...
						<li id="item4" role="presentation"><a href="javascript:void(0);" id="prepaid_tab"><%-$.i18n.t('app.wealth.prepaidcard')%></a></li>						
						<li id="item5" role="presentation" class="active"><a href="javascript:void(0);" id="loan_tab"><%-$.i18n.t('app.wealth.loans')%></a></li>						
				  	</ul>
				  	<br/>
				  	<div class="tab-content">
						<div role="tabpanel" class="tab-pane active" id="Accounts">
							<%if(isNull(loanListArray)) { %>
								<p class="help-block error-message" style="color:#fff; text-align:center;"><%-$.i18n.t('app.product.general.noloancardfound')%></p>
							<% } else if(loanListArray.length > 0){%>
								<div class="c-cards-stack">       
								<ul class="stack-container">
									<% _.each(loanListArray,function(data, index){%>
											<% var loantype = data.accType;console.log(loantype); els.set("loanType",data.accType)%>
											<% if(loantype == "TL") { %>
												<li class="c-cards-stack-item card card-default loan" data-url="#/termloan/<%- index%>" availBalance="<%-data.outStandingAmount%>" currency="<%- data.currencyCode%>">
												<p class="name"><%- data.accountShortName%></p>
												<p class="numb small text-muted"><%- data.accountNumber%></p>
												<div class="amt text-center">
													...
													<% if(els.get("dashbaordShowHideFlag") == "Show") { %>
														<% if(data.outStandingAmount >= 0) {%>												  	
															<span class="num"><%- checkAmount($.formatNumber(data.outStandingAmount, {format:"#,###.00", locale:"us"}))%></span> <span class="small cur dCur text-muted"><%- data.currencyCode %></span>
														<% } else { %>
															<span class="num neg"><%- checkAmount($.formatNumber(data.outStandingAmount, {format:"#,###.00", locale:"us"}))%></span> <span class="small cur dCur text-muted"><%- data.currencyCode %></span>
														<% } %>
													<% } else if(els.get("dashbaordShowHideFlag") == "Hide") { %>
														<% if(data.outStandingAmount >= 0) {%>												  	
															<span class="num"><%- data.maskedOutStandingAmount %></span> <span class="small cur dCur text-muted"><%- data.currencyCode %></span>
														<% } else { %>
															<span class="num neg"><%- data.maskedOutStandingAmount %></span> <span class="small cur dCur text-muted"><%- data.currencyCode %></span>
														<% } %>
													<% } %>
											...
											<%} else if(loantype == "RL") { %>
												<li class="c-cards-stack-item card card-default loan" data-url="#/revolveloan/<%- index%>" availBalance="<%-data.totalLimit%>" currency="<%- data.currencyCode%>">
												<p class="name"><%- data.accountShortName%></p>
												<p class="numb small text-muted"><%- data.accountNumber%></p>
												<div class="amt text-center">
													...
													<% if(els.get("dashbaordShowHideFlag") == "Show") { %>
														<% if(data.totalLimit >= 0) {%>												  	
															<span class="num"><%- checkAmount($.formatNumber(data.totalLimit, {format:"#,###.00", locale:"us"}))%></span> <span class="small cur dCur text-muted"><%- data.currencyCode%></span>
														<% } else { %>
															<span class="num"><%- checkAmount($.formatNumber(data.totalLimit, {format:"#,###.00", locale:"us"}))%></span> <span class="small cur dCur text-muted"><%- data.currencyCode%></span>
														<% } %>
													<% } else if (els.get("dashbaordShowHideFlag") == "Hide") { %>
														<% if(data.totalLimit >= 0) {%>												  	
															<span class="num"><%- data.maskedTotalLimit%></span> <span class="small cur dCur text-muted"><%- data.currencyCode%></span>
														<% } else { %>
															<span class="num"><%- data.maskedTotalLimit%></span> <span class="small cur dCur text-muted"><%- data.currencyCode%></span>
														<% } %>
													<% } %>
											...
											<% } else if(loantype == "DL") { %>
												<li class="c-cards-stack-item card card-default loan" data-url="#/demandloan/<%- index%>" availBalance="<%-data.outStandingAmount%>" currency="<%- data.currencyCode%>">
												<p class="name"><%- data.accountShortName%></p>
												<p class="numb small text-muted"><%- data.accountNumber%></p>
												<div class="amt text-center">
													<p class="small m0 text-muted"><%-$.i18n.t('app.product.loan.currentoutbal')%></p>										  		
												...
												<p class="text-right m0"><span class="logo-xs"></span></p>
											<% } else if(loantype == "LT") { %>
												<li class="c-cards-stack-item card card-default loan" data-url="#/latrloan/<%- index%>" availBalance="<%-data.totalLimit%>" currency="<%- data.currencyCode%>">
												<p class="name"><%- data.accountShortName%></p>
												<p class="numb small text-muted"><%- data.accountNumber%></p>
												<div class="amt text-center">
													<p class="small m0 text-muted"><%-$.i18n.t('app.product.loan.totAvailLimt')%></p>										  		
												...
												<p class="text-right m0"><span class="logo-xs"></span></p>
											<% } else if(loantype == "OD") { %>
												<li class="c-cards-stack-item card card-default loan" data-url="#/overdraftloan/<%- index%>" availBalance="<%-data.principalBalance%>" currency="<%- data.currencyCode%>">
												<p class="name"><%- data.accountShortName%></p>
												<p class="numb small text-muted"><%- data.accountNumber%></p>
												<div class="amt text-center">
													<p class="small m0 text-muted"><%-$.i18n.t('app.product.loan.totAvailLimt')%></p>										  		
													...
													<% } else if(els.get("dashbaordShowHideFlag") == "Hide") { %>
														<% if(data.principalBalance >= 0) {%>												  	
															<span class="num"><%- data.maskedPrincipalBalance%></span> <span class="small cur dCur text-muted"><%- data.currencyCode%></span>
														<% } else { %>
															<span class="num"><%- data.maskedPrincipalBalance%></span> <span class="small cur dCur text-muted"><%- data.currencyCode%></span>
														<% } %>
...
$(document).ready(function() {
	$('.nav-tabs').animate({scrollLeft: $('#item5').position().left});
	if(!isNull(getAdvertisementDTO().advertisement) && getAdvertisementDTO().advertisement.length > 0) {
		loadOffersPopUp();
	}
});
