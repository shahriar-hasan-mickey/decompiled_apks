	...
	<div class="headerRight">
		<a href="javascript:void(0);" class="btn-right notifications" id="pushNotifications">
			...
			<% if(!isNull(els.get("unreadMsgCount")) && (els.get("unreadMsgCount") != "0")) { %>
				<span class="badge"><%- els.get("notificationCount")%></span>
			<% } %>
		</a>
            	...
            	<div class="userInfo">
          		...
          		<div class="dashTab">
					<ul class="nav nav-tabs" role="tablist">
						<li role="presentation"><a href="javascript:void(0);" id="accounts_tab"><%-$.i18n.t('app.wealth.accounts')%></a></li>
						<li role="presentation"><a href="javascript:void(0);" id="termdeposit_tab"><%-$.i18n.t('app.wealth.termdeposit')%></a></li>
						<li role="presentation" class="active"><a href="javascript:void(0);" id="creditcard_tab"><%-$.i18n.t('app.wealth.creditcard')%></a></li>
						<li role="presentation"><a href="javascript:void(0);" id="prepaid_tab"><%-$.i18n.t('app.wealth.prepaidcard')%></a></li>
				  	...
				  	<div class="tab-content">
						<div role="tabpanel" class="tab-pane active" id="Accounts">
							<% if(isNull(creditCardListArray)) { %>
								<p class="help-block error-message" style="color:#fff; text-align:center;"><%-$.i18n.t('app.product.general.nocardfound')%></p>
							<% } else if(creditCardListArray.length > 0 ){ %>
								<div class="c-cards-stack">       
								<ul class="stack-container">
									<% _.each(creditCardListArray,function(data, index){ %>
										<li class="c-cards-stack-item cc_summary_design cc<%- data.productCode %>" currency="<%- data.currencyCode %>" outStandingAmount="<%- data.outStandingCCAmount %>" amount="<%- data.availableCreditLimit %>" productCode="<%- data.productCode %>" data-url="#/ccsummaryview/<%-index%>" availBalance="<%- data.availableBalance%>" cardName="<%- data.cardHolderName%>">
											<div class="cc_container">
												<p class="text-center ccno_wrap">
													<span class="cc_name"><%- data.cardHolderName%></span>
													<bdi>
													<% var txnaccnum = data.accountNumber;  var txnAccNumArrr=txnaccnum.match(/.{1,4}/g); console.log(txnAccNumArrr.join(' ')); %>
													<!--<span class="ccno"><%-txnAccNumArrr.join(' ')%></span>-->
												...
												<div class="row">
													<div class="col-xs-6 credit_card_val_al">
														<div class="amt">
															<p class="small m0"><%-$.i18n.t('app.creditcard.general.creditavailable')%></p>
															<% if(els.get("dashbaordShowHideFlag") == "Show") { %>
																...
																<span class="small cur dCur"><%- data.currencyCode %></span>
													...
													<div class="col-xs-6 credit_card_val_ob">
														<div class="amt">
															...
															<% if(els.get("dashbaordShowHideFlag") == "Show") { %>
																<span class="num"><%- checkAmount($.formatNumber(data.outStandingCCAmount, {format:"#,###.00", locale:"us"}))%></span>							
																<span class="small cur dCur"><%- data.currencyCode %></span>
															<% } else if(els.get("dashbaordShowHideFlag") == "Hide") { %>
																<span class="num"><%- data.maskedOutStandingCCAmount%></span>							
																<span class="small cur dCur"><%- data.currencyCode %></span>
															<% } %>											  	
														</div>
													</div>
										...
										<!--<li class="c-cards-stack-item card card-default acc card-CC card-CC-gold visa-CC" data-url="#/ccsummaryview/<%-index%>">									
											<p class="m0"><%- data.productName%><span class="logo-xs pull-right"></span></p>
											...
											<div class="amt">
												<p class="small m0 text-muted"><%-$.i18n.t('app.creditcard.general.creditavailable')%></p>										  			
												<span class="num"><%- $.formatNumber(data.availableCreditLimit, {format:"#,###.00", locale:"us"})%></span> <span class="small cur dCur text-muted"><%- data.currencyCode %></span>
											</div>
										</li>-->
									<%});%>
								</ul>							
