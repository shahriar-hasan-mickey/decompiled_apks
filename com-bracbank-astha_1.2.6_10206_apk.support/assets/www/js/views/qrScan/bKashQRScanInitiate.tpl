<%
	...
	if(!isNull(ccListDTO)) {
		ccListArray = ccListDTO.filter(function(array) {
			return (array.merchantType != "" && array.bkashQRFlag == "Y");
		});
	}	
	...
	if(!isNull(myaccounts)) {
		casaListArray = myaccounts.filter(function(array) {
			return (array.bkashQRFlag == "Y");
		});
	}
...
<div class="pagewrapper" id="appwrapper" >
				...
				<form>    
					<div class="media payAcc">
						<div class="media-left">
							...
							<img src="images/brand/biller/bKash_bangla_logo.png" class="img2x img-circle">
						</div>
						<div class="media-body">
							<p><%- els.get("merName")%></p>
							<p class="small">
							<span class="text-muted small"> Wallet Number: </span>
							<span class="text-primary"><%-els.get("bKashQRWalletNumber")%></span>
							</p>
						</div>
					</div>
					<br/>
					...
					<div class="form-group">
						<div class="row">
			                <div class="col-xs-5 pr5">
			                   <label class="m0 mt10" for="bkashqrscan_amount"><%-$.i18n.t('app.qrcode.entertransferamount')%></label>
			                </div>
			                <% if(isNull(els.get("txnAmt"))) { %>
								<div class="col-xs-7 pl5">
									<input id="bkashqrscan_amount" autocomplete='off' type="tel" data-type="currency" pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" onkeyup="withoutstartZero(event,'bkashqrscan_amount')" class="form-control numericlengthcontrol transferAmountClass text-right">
								</div>
							<% } else { %>
								<div class="col-xs-7 pl5">
									<input id="bkashqrscan_amount" autocomplete='off' type="tel" data-type="currency" pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" onkeyup="withoutstartZero(event,'bkashqrscan_amount')" class="form-control numericlengthcontrol text-right transferAmountClass" value="<%-checkAmount($.formatNumber(els.get("txnAmt"), {format:"#,###.00", locale:"us"}))%>" disabled>
								</div>
							<% } %>
			            </div>
						<div class="form-group has-error">
								<p class="help-block error-message has-error" style="color:#a94442" id="bakshQR_transAmountError"></p>
	 						</div>
 						<div id="bankFeeDiv" class="row">
 							<div class="col-xs-5 pr5"><label>Service Fee</label></div>
 							<div class="col-xs-7 pl5"><p id="bkashQRFee" class="text-right"></p></div>
 						</div>
						<div class="row" id="qr_tips">
			                ...
			                <div class="col-xs-7 pl5">                    
			                    <input type="tel" autocomplete='off' class="form-control text-right m0 qr_value numericlengthcontrol transferAmountClass" data-type="currency" pattern="^\$\d{1,3}(,\d{3})*(\.\d+)?$" id="bkash_qrtips_amount" value="">                    
			                </div>
			            </div>
										
						<div class="form-group has-error">
								<p class="help-block error-message has-error" style="color:#a94442" id="bkashQR_tipsAmountError"></p>
	 						</div>
						
			    		
		                <div class="row qr_total" id="paymentAmtDiv">
		                    <div class="col-xs-6">
		                        <p class="small qr_total_left"><%-$.i18n.t('app.qrcode.paymentamount')%></p>
		                    </div>
		                    <div class="col-xs-6 text-right">
		                        <p class="amt m0" id="bkash_qrtotamt">
		                        </p>
		                    </div>
						</div>
							
		    		<div class="form-group">
						<label for="bkash_crdaccid"><%-$.i18n.t('app.qrcode.payfromcard')%></label>
						<a style="outline:none;" href="javascript:void(0);" class="acc_link" id="bkash_crdaccid"  data-toggle="modal" data-target="#bkash_card_account"><div id="seltpayval" style="width:90%;height:5px;">Select Account/Card</div><span class="glyphicon glyphicon-triangle-bottom pull-right" aria-hidden="true"></span></a> 
						<div class="form-group has-error">
					...
					<!--<div class="form-group">
		              	<label class="check-label">
		                	<input type="checkbox" name="transfer" id="termsand"> I agree to the <a href="javascript:void(0);" id="bKashQRTandCPopup">Terms and Conditions</a>
					...
					<div class="row">
						<div class="col-xs-6 pr5">
							<button type="button" class="btn btn-default btn-block" id="bkash_qrscan_cancel"><%-$.i18n.t('app.qrcode.btncancel')%></button>
						</div>
						<div class="col-xs-6 pl5">
							<button type="button" class="btn btn-primary btn-block" id="bksh_qrscan_confirm"><%-$.i18n.t('app.qrcode.btnconfirm')%></button>
						</div>
					</div>
				</form>
			</section>
    ...
    <!-- Modal -->
    <div class="modal fade trans_popup" id="bkash_card_account" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
			            ...
			            <div class="qr_acc_outer">
			              <% if(!isNull(dcListArray )) { %>
			                ...
			                <ul class="list-unstyled">
			                    ...
			                    <li class="list-group-item bkash_acccardnumtype" acctype="DC" frName="<%-data.productName%>" merchantType="<%-data.merchantType%>" value="<%- data.id%>" Accide="<%- data.id%>" id="dc-<%- index%>" data="<%- data.currencyCode%>">
			               ...
			               <%}%>	
			                ...
			                <ul class="list-unstyled">
			                    ...
			                    <li class="list-group-item bkash_acccardnumtype" acctype="CC" frName="<%-data.productName%>" merchantType="<%-data.merchantType%>" value="<%- data.id%>" Accide="<%- data.id%>" id="cc-<%- index%>" data="<%- data.currencyCode%>">
			                           <label class="radio-label">
			                            <div class="row">
			                                <div class="col-xs-2 first-col">
			                                 <% if(data.merchantType == "VISA" ) { %>
			                ...
			                <ul class="list-unstyled">
			                    ...
			                    <li class="list-group-item bkash_acccardnumtype" acctype="PC" frName="<%-data.productName%>" merchantType="<%-data.merchantType%>" value="<%- data.id%>" Accide="<%- data.id%>" id="pc-<%- index%>" data="<%- data.currencyCode%>">
			                           <label class="radio-label">
			                            <div class="row">
			                                <div class="col-xs-2 first-col">
			                                 <% if(data.merchantType == "VISA" ) { %>
